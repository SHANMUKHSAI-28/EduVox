...
export const exportUniversitiesPDF = (universities, userProfile = null, userEmail = '') => {
  ...
  doc.text('EduVox - University Shortlist', margin, 25);
  ...
  doc.text('Generated by EduVox - Your AI Study Abroad Assistant', margin, footerY);
  doc.text('Visit: www.eduvox.com | Contact: support@eduvox.com', margin, footerY + 5);
  
  // Save the PDF
  const fileName = `EduVox_University_Shortlist_${date.replace(/\//g, '-')}.pdf`;
  doc.save(fileName);
};
...
export const exportComparisonPDF = (universities, userEmail = '') => {
  ...
  doc.setFontSize(18);
  doc.setTextColor(59, 130, 246);
  doc.text('EduVox - University Comparison', margin, 25);
  ...
  // Save the PDF
  const fileName = `EduVox_University_Comparison_${date.replace(/\//g, '-')}.pdf`;
  doc.save(fileName);
};
