...
const UniversityFilters = ({ onFiltersChange, loading = false, onUpgradePrompt }) => {
  const { canPerformAction, planType } = useSubscriptionLimits();
  const isAdvancedFeatureDisabled = planType === 'free';
  ...
  const handleFilterChange = (name, value) => {
    // Check if this is an advanced filter that requires subscription
    const advancedFilters = ['tuitionMin', 'tuitionMax', 'rankingMax'];
    if (advancedFilters.includes(name) && planType === 'free') {
      if (onUpgradePrompt) {
        onUpgradePrompt();
      }
      return;
    ...
    <div className="bg-white/70 backdrop-blur-xl rounded-3xl p-6 border border-white/20 shadow-xl">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">          <FormInput
            ...
            disabled={isAdvancedFeatureDisabled}
          />          <FormInput
